...
int main(int argc, char* argv[]) {
    if (argc < 5) {
        cout << "Usage:\n        " << argv[0] << " [in_d64] [out_d64] [manifest (name location t:s [u])] [out_asm] [options]" << endl;
        cout << "Options:" << endl;
    ...
    string in_path = argv[1], out_path = argv[2], manifest_path = argv[3], asm_path = argv[4], report_path;
    ...
    for (auto& d : definitions) {
        ...
        int nblocks = blocks_needed_for(d.path);
        uint32_t size = file_size_of(d.path);
        vector<pair<int,int>> blocks = reserve_run(disk, order, pos_index, d.m_track, d.m_sector, nblocks, "manual placement for '" + d.name + "'");
    ...
    if (blob.size() > manifest_blocks.size() * 256) {
        cerr << "Error: manifest table is " << blob.size() << " bytes, but only "
             << manifest_blocks.size() * 256 << " reserved (manifest_block_count="
             << manifest_block_count << ")." << endl;
        return 1;
    }
    ...
    cout << "OK: " << (placements.size() + unlisted_placements.size()) << " assets placed. Wrote " << out_path << ", " << report_path << (!report_path.empty()?", ":"") << asm_path << endl;
    return 0;
}
